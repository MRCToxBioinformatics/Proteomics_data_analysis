...
We can also look into this relationship at the tag level using `plot_missing_SN_per_sample`. In this case, there is no tag which appears to have a high proportion of missing values when signal:noise > 40.
...
For PSM to protein summarisation, we will use naive 'sum' summarisation (`MSnbase::combineFeatures(method = 'sum')`). This approach can not handle missing values, but we have so few that we can simply remove them. 

Here, we will remove PSMs with any missing values
