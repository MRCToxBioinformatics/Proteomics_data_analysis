...
> ### Discussion
> How would you interpret this volcano plot. What are your conclusions about the
> proteins with significantly positive RNase +/- ratios

> Solution

```{r class.source = 'fold-hide', eval=FALSE, fig.show='hide'}
# LFQ provides relative abundances for each sample. So the ratio for RNase +/- is
# the ratio between two relative abundances. We therefore have the limit our 
# interpretion to changes in the relative abundances and can not be certain about
# changes in absolute abundances.

# However, here, the RNase +/- samples were taken from the same starting material and RNase
# treatment can only deplete the material. Significantly higher abundance in RNase +
# for a given protein therefore represents a relatively smaller effects of RNase +
# compared to the overall depletion this causes for the sample as a whole.

# This is an experiment where relative abundances are difficult to interpret!
```
...
deqms_results %>%
  ...
  tally()
```
If we compare the p-values, we see there is no consistent difference between the two methods in this case.
```{r}
