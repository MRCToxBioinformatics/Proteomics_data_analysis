...
Here, we will start with the SILAC data processed in [Processing and QC of SILAC data](https://mrctoxbioinformatics.github.io/Proteomics_data_analysis/Markdowns/SILAC.html). Please see the previous notebook for details of the experimental design and aim and data processing. 
...
First, we read in the protein-level ratios obtained in the above notebooks. We will filter out proteins with too few quantification values across the 4 replicates. Here, we allow a maxmimum of 50% missing, e.g 2/4 replicates.
```{r}
...
In this case the relationship between peptide count and variance is only really apparent when the minimum number of peptides from which a protein-level ratio is obtained is very low. Typically, one might remove quantifications where there is just a single peptide for a protein. Here, we have kept them, and we can refer to the `count` column in the final results object if we want to check the minimum number of peptides observed per sample.
...
p <- ggplot(silac_compare_tests) +
  ...
  geom_abline(slope=1) +
  theme_camprot(border=FALSE) +
  labs(x='t.test p-value', y='DEqMS p-value')
