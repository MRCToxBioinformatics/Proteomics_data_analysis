...
First, we read in the protein-level ratios obtained in the above notebooks. We will filter out proteins which are not present in at least 3/4 replicates.
```{r}
lfq_protein <- readRDS('./results/lfq_prot_robust_glyco_norm.rds')
```


First, we need to calculate the RNase +/- ratios, since this is the value we want to normalise in this case. We will filter out proteins which are not present in at least 3/4 replicates.
```{r}
...
lfq.t.test.all <- lfq_protein_tidy %>%
  group_by(protein) %>%
  do(tidy(t.test(value ~ 1, data = ., alternative='two.sided')))
```
...
First, we create the `MArrayLM` object. 
```{r}
lfq_dat <- lfq_protein_ratios  %>% 
  exprs()

# Performing the equivalent of a one-sample t-test, so the design matrix is just an intercept
lfq_design <- cbind(Intercept = rep(1, ncol(lfq_dat))) 

lfq_fit <- lmFit(lfq_dat, design)
lfq_efit <- eBayes(lfq_fit)
```
...
the samples, so our `count` column is the minimum number of peptides per protein.  
```{r}
lfq_protein_ratio_long <- lfq_protein_ratios %>%
  exprs() %>%
  data.frame() %>%
  tibble::rownames_to_column('Master.Protein.Accessions') %>%
  pivot_longer(cols=-Master.Protein.Accessions, values_to='protein_ratio', names_to='sample')
...
lfq_min_pep_count <- camprotR::count_features_per_protein(lfq_pep_res) %>%
  merge(lfq_protein_ratio_long, by=c('Master.Protein.Accessions', 'sample')) %>%
  filter(is.finite(protein_ratio)) %>%  # We only want to consider samples with a ratio quantified
  group_by(Master.Protein.Accessions) %>%
  summarise(min_pep_count = min(n))
...
```{r, fig.height=6, fig.width=6}
lfq.deqms_results <- outputResult(lfq_efit_deqms, coef_col = 1)
